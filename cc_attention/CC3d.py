...
class CrissCrossAttention3D(nn.Module):
    ...
    def forward(self, x):
        ...
        proj_key_D = proj_key.permute(0, 2, 3, 1, 4).contiguous().view(m_batchsize * height * width, -1,
                                                                       depth)  # b*h*w-c-d
        ...
        proj_value_H = proj_value.permute(0, 3, 4, 1, 2).contiguous().view(m_batchsize * width * depth, -1,
                                                                           height)  # bchwd->bwdch
        proj_value_W = proj_value.permute(0, 2, 4, 1, 3).contiguous().view(m_batchsize * height * depth, -1,
                                                                           width)  # bchwd->bhdcw
        proj_value_D = proj_value.permute(0, 2, 3, 1, 4).contiguous().view(m_batchsize * height * width, -1,
                                                                           depth)  # bchwd->bhwcd

        # batch matrix-matrix
        inf_holder = self.INFH(m_batchsize, height, width, depth).to(x.device)  # > bw-h-h 
        if self.verbose: print('inf', inf_holder)
...
class CrissCrossAttention3D1(nn.Module):
    ...
    def forward(self, x):
        ...
        att_H = concate[:, :, :, :, 0:height].permute(0, 2, 3, 1, 4).contiguous().view(m_batchsize * width * depth,
                                                                                       height, height)
        att_W = concate[:, :, :, :, height:height + width].permute(0, 1, 3, 2, 4).contiguous().view(
            m_batchsize * height * depth, width, width)
...
class CrissCrossAttention3D2(nn.Module):
    ...
    def forward(self, x):
        ...
        # b*h*w-d-c ,b*h*w-c-d ->b*h*w-d-d
        energy_D = (torch.bmm(proj_query_D, proj_key_D) + self.INFD(m_batchsize, height, width, depth).to(
            x.device)).view(m_batchsize, height, width, depth, depth)  #
        if self.verbose: print('ew', energy_W)
